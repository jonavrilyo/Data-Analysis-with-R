...
<li> No flight is delayed more than 2 hours. *Correct Answer* </li> 
...
sfo_feb_flights %>%
  summarize(mean_ad = mean(arr_delay), median_ad = median(arr_delay), iqr_ad = IQR(arr_delay), sd_ad = sd(arr_delay), max_ad = max(arr_delay))
```
...
<li> Delta and United Airlines *Correct Answer* </li>
...
<li> Median would be more reliable as the distribution of delays is skewed. *Correct Answer* </li> 
...
<li> There is an overall postive association between distance and average speed. *Correct Answer* </li> 
...
nycflights %>%
  group_by(dep_type) %>%
  summarize(ot_arr_rate = sum(arr_type == "on time") / n())
print("0.183")
```